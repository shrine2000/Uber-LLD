...
class Driver(User, TripObserver):
    def __init__(self, user_id: str, name: str, contact: str, vehicle: Vehicle, current_location: Location):
    ...
    def on_update(self, trip: 'Trip') -> None:
        print(f"Driver {self.name} notification: Trip {trip.id} status changed to {trip.status.name}")
