...
from ridesharing.strategy.pricing_strategy import PricingStrategy, VehicleBasedPricingStrategy
from ridesharing.strategy.driver_matching_strategy import DriverMatchingStrategy, NearestDriverMatchingStrategy
...
class TripManager:
    ...
    def request_trip(self, rider: Rider, pickup: Location, dropoff: Location, ride_type: RideType) -> Optional[Trip]:
        ...
        print(f"Trip {trip_id} created and assigned to driver {driver.name} with fare ${fare:.2f}")
    ...
    def get_trip_history(self, user_id: str) -> List[Trip]:
        return [trip for trip in self.trips if trip.rider.id == user_id or (trip.driver and trip.driver.id == user_id)]
